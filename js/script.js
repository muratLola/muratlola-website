...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (hdr) {
    const onScr = () => hdr.classList.toggle('scrolled', window.scrollY > 8);
  ...
  if (form && !form.hasAttribute('action') && status) {
    form.addEventListener('submit', e => {
      e.preventDefault();
      status.textContent = 'Form henüz bir servise bağlı değil. Yukarıdaki e-posta en hızlı yol.';
    });
  }
  ...
  if (badge && !reduced) {
    ...
    badge.textContent = '';
    let i = 0;
    const type = () => { if (i < full.length) { badge.textContent += full[i++]; setTimeout(type, 55); }};
    setTimeout(type, 600);
  }
  ...
  document.querySelectorAll('[data-count]').forEach(el => {
    ...
    const io2 = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting) {
        let count = 0;
        const step = () => { count = Math.min(count + Math.ceil(target/40), target); el.textContent = count + (el.dataset.suffix || ''); if (count < target) requestAnimationFrame(step); };
